...
##### 方案 B：上传 **整个 nginx 配置文件夹** （全量覆盖，完整同步）

你本地已经下载了完整的 `nginx` 文件夹，修改完后， **一次性全部上传覆盖服务器** ：

```
scp -r ./nginx root@你的服务器IP:/etc/
```

- 把你 **本地整个 nginx 文件夹** （包含 nginx.conf + conf.d/ 所有配置 + 全部文件）
- **完整覆盖服务器 /etc/nginx**
